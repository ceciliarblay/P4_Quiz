 ...
 * Si todo va bien, la promesa se satistafe y devuelv el valor id a usar.
 * 
...
/**
 * Esta funcion devuelve una promesa que cuando se cumple, porporciona el texto introducido
 * Entonces la llamada a then hay que hacer la promesa devuelta sera:
 *        .then(answer => {...})
 *
 * Tambien colorea en rojo el texto de la pregunta, elimina espacions al principio
 *
 * @param rl Objeto readline usado para implementar el CLI.
 ...
 // } 
  ...
  const playOne = () =>{
    ...
    if (toBeResolved.length === 0){
      log(socket, 'No quedan más preguntas. Has completado el Quiz.');
      log(socket, 'Tu puntuación final ha sido de: ' +score+' aciertos.');
      rl.prompt();
      ...
      .then(quiz => {
        ...
        if(a.trim().toLowerCase() === quiz.answer.toLowerCase()){
          score ++;
          log(socket, 'Llevas '+score+' respuestas correctas.');
...
exports.creditsCmd = (socket, rl) => {
  ...
  log(socket, 'Cecilia Rodrigo Blay', 'green');
  rl.prompt();
};
